...
\begin{figure}[H]
	\tmpframe{\includegraphics[width=\linewidth]{BAC-DES10}}
...
\begin{figure}[H]
	\tmpframe{\includegraphics[width=\linewidth]{BAC-DES3}}
...
\begin{figure}[H]
	\tmpframe{\includegraphics[width=\linewidth]{BAC-DES4}}
...
Paralelizmus na úrovni systémů lze přirovnat k \emph{úkolovému paralelizmu}, kde každé vlákno zpracovává rozdílný systém. Tento způsob má opět omezení -- systémy nemohou přistupovat ke stejným komponentám (stejných entit). 

Tyto způsoby paralelizmu je možno kombinovat, ale v některých situacích není možné použít ani jeden z nich. Další nevýhodou je nemožnost používat metody pro manipulaci entit (aktivita, rušení entit aj.) z několika vláken, jelikož tyto metody používají globální kontext. Jedním řešením je použití zámků (např. vzájemné vyloučení -- \uv{mutex}), které budou tyto metody chránit před problémy souběhu (\uv{race condition}). Zámky jsou čistým způsobem, jak implementovat bezpečný paralelizmus, ale vysokém počtu operací zamčení a odemčení je možné ztratit výkon, který byl získán paralelním zpracováním \footnote{Pokud jsou operace chráněné zámky používány často, je možné paralelní aplikaci redukovat na aplikaci sekvenční.}. Kvůli těmto nevýhodám je součástí návrhu také třetí metoda paralelizace -- pomocí \emph{množin změn}.
...
Základem podsystému \emph{množin změn} (\uv{changeset}), jehož diagram lze vidět na obr. \ref{Fig:DESActionDiag}, jsou \emph{akce}. \emph{Akce} 

\emph{Akce} jsou rozděleny do dvou typů -- změny metadat a změny komponent. 

pokud potreba akce
Pripady napr pripojeni skriptovaciho jazyka
mutex na normalni createEntity
